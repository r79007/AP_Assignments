        ...
        if(VaccinationStatus.vaxStatusMap.containsKey(uniqueID)){
            if(hs.vax.equals(VaccinationStatus.vaxStatusMap.get(uniqueID).vax)){
                if(VaccinationStatus.vaxStatusMap.get(uniqueID).numDosesRemaining()!=0){
                    ...
                    ArrayList<HospitalSlot> newList=HospitalSlot.HospitalSlotMap2.get(hs.HospitalID);
                    if(newList!=null){
                        for(int i=0;i< newList.size();i++){
                            if(newList.get(i).HospitalID==hs.HospitalID && newList.get(i).vax.equals(hs.vax) && newList.get(i).dayNumber==hs.dayNumber){
                                //newList.get(i).quantity=this.hs.quantity;
                                newList.get(i).quantity--;
                            }
                        }
                        HospitalSlot.HospitalSlotMap2.put(hs.HospitalID,newList);
                    }
                    VaccinationStatus.dueDateMap.put(uniqueID,hs.dayNumber+Vaccine.vaccineMap.get(hs.vax).gapBwDoses);
//                    System.out.println("*****************");
//                    System.out.println(VaccinationStatus.vaxStatusMap.get(uniqueID).numDosesRemaining());
//                    System.out.println("*****************");
                    ...
                    if (opt == 2) {
                        ...
                        if (opts != null && opts.size() != 0) {
                            ...
                            if (options != null && options.size() != 0) {   //Should be && here
                                ...
                                giveVaccine(uniqueID,HospitalSlot.HospitalSlotMap2.get(hospID).get(slot));
                            } else {
                                System.out.println("No slots available");
                            }
                        }
