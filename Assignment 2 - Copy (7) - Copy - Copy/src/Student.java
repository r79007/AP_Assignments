...
public class Student implements Discussions{
    ...
    public ArrayList<Integer> viewPendingAssessments(){
        ...
        for(int i=0;i<=Main.assignmentIDs;i++){
            if((Assignment.getAssignmentHashMap().containsKey(i) && Assignment.getAssignmentHashMap().get(i).isClosed()==false) || (Quiz.getQuizHashMap().containsKey(i) && Quiz.getQuizHashMap().get(i).isClosed()==false)) {
                if (submissions.isEmpty() == true || (submissions.isEmpty() == false && !submissions.containsKey(i))) {
                    if (Assignment.getAssignmentHashMap().containsKey(i)) {
                        System.out.println("ID " + Assignment.getAssignmentHashMap().get(i).getAssessmentID() + " " + Assignment.getAssignmentHashMap().get(i).getProblemStatement() + " Max Marks: " + Assignment.getAssignmentHashMap().get(i).getMarks());
                        check.add(i);
